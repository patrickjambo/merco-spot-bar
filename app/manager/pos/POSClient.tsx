...
function POSContent({ initialProducts }: { initialProducts: Product[] }) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const initialTable = searchParams.get("table") || "1";

  // Products are server-rendered and passed in, so the grid is populated on first paint (no loading flash).
  ...
  useEffect(() => {
    ...
    // Background refresh: server already rendered fresh products, but re-sync stock
    // in case the catalog changed since this page was last navigated to.
  ...
  const addToCart = (product: Product, saleType: "unit" | "packet") => {
    ...
    // NOTE: On mobile we intentionally keep the product menu visible after adding an
    // item (instead of auto-opening the full-screen cart) so the cashier can keep
    // adding products. The sticky "View Order" bar at the bottom shows the running
    // total and opens the cart for review/checkout when they are ready.
  };
    ...
    <div className="h-[100dvh] w-full bg-zinc-100 dark:bg-zinc-950 flex flex-col overflow-hidden relative">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 relative">
        ...
        <div className={`${isCartOpenMobile ? 'flex absolute inset-0 z-30' : 'hidden'} lg:flex lg:relative w-full lg:w-[400px] lg:min-w-[350px] h-full bg-white dark:bg-zinc-900 lg:border-l border-zinc-200 dark:border-zinc-800 flex-col shadow-2xl shrink-0`}>
          ...
          {/* Totals & Action (footer is pinned; safe-area padding keeps the Confirm button
              above the phone's home indicator / browser chrome so it never needs scrolling) */}
          <div className="p-3 lg:p-5 bg-zinc-50 dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800 space-y-3 lg:space-y-4 shrink-0 pb-[calc(1.25rem+env(safe-area-inset-bottom))] lg:pb-5">
...
export default function POSClient({ initialProducts }: { initialProducts: Product[] }) {
  return (
    <Suspense fallback={<div className="h-[100dvh] w-full bg-zinc-100 flex items-center justify-center p-6"><p>Loading POS...</p></div>}>
      <POSContent initialProducts={initialProducts} />
    </Suspense>
  );
}
