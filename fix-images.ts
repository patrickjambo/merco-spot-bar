...
async function run() {
  ...
  for (const p of products) {
    if (p.imageUrl && p.imageUrl.startsWith('/products/')) {
      ...
      const newUrl = (imageMapping as Record<string, string>)[fileName] || defaultImage;
      ...
      console.log(`Updated ${p.name}`);
        ...
        // Also update ones with completely missing images
        await prisma.product.update({
            where: { id: p.id },
            data: { imageUrl: defaultImage }
        });
        console.log(`Updated ${p.name} with default`);
    } else if (p.imageUrl && p.imageUrl.includes('unsplash.com/photo-1600041162228')) {
        await prisma.product.update({
            where: { id: p.id },
            data: { imageUrl: defaultImage }
        });
        console.log(`Updated broken 404 unsplash image on ${p.name}`);
    }
  }
}
